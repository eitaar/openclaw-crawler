...
- **Entry rewriting**: users can customize per-entry line templates for hook messages and edit individual recorded entries before sending.
...
- **SPA compatibility**: content script listens to History API/navigation/meta changes so URL updates (e.g., search pages) are recorded without full reload.
